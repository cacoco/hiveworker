...
class HiveWorkerServiceImpl(contextOption: Option[ApplicationContext]) extends Service[Request, Response] with HiveWorkerService {
  ...
  lazy val jobFlowService: Option[JobFlowService] =
    contextOption map { _.getBean("jobFlowService").asInstanceOf[JobFlowService] }
